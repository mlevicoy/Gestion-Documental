<?PHP
	...
	if(!isset($_POST["cargador"])){				
		...
		//Creamos un subInforme
            ...
            $consulta9 = "select count(*) as ctdad_variosSubInformes from variossubinformes where nombre_variosSubInformes = '".$nombre_variosSubInformes."'";
            ...
            if($ctdad_variosSubInformes == 1){
                ...
                $resultado13->bind_result($nombre_informe);
                $resultado13->fetch();
                ...
                if(!mkdir($ruta_variosSubDocumentos, 0777, true)){
                    ...
                    $consulta10 = "insert into variossubinformes (nombre_variosSubInformes) values ('".mb_strtoupper($nombre_variosSubInformes,'UTF-8')."')";
                    $resultado10 = $conexion_db->prepare($consulta10);
                    $resultado10->execute();
                
                    $consulta11 = "select id_variosSubInformes from variosSubinformes where nombre_variosSubInformes = '".mb_strtoupper($nombre_variosSubInformes,'UTF-8')."'";
		...
		//Cuando el nombrevariossubinforme ya esta en la base de datos, se consulta si existe el directorio
            ...
            $resultado12->execute();
            $resultado12->store_result();
            $resultado12->bind_result($nombreCorto_contrato);
            $resultado12->fetch();                
            ...
            $consulta14 = "select nombre_variosSubInformes from variossubinformes where id_variosSubInformes = ".$_POST["variosSubInforme"];
            $resultado14 = $conexion_db->prepare($consulta14);
            $resultado14->execute();
            $resultado14->store_result();
            $resultado14->bind_result($nombre_variosSubInformes);
            $resultado14->fetch();
            ...
            $consulta8 = "select id_variosSubInformes, nombre_variosSubInformes from variossubinformes where id_variosSubInformes = ".$_POST["variosSubInforme"];
            $resultado8 = $conexion_db->prepare($consulta8);
            $resultado8->execute();
            $resultado8->store_result();
            $resultado8->bind_result($variosSub_documento, $nombre_variosSubInformes);
            $resultado8->fetch();
        }
		...
		if($identificador_subInforme != -1){
			...
			$nombre_documento = $identificador_numero.".".html_entity_decode(sanear_string($nombre_variosSubInformes))."_N".$identificador_numero;			
		}		
